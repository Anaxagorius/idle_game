  ...
  Save.applyData = function (data) {
    ...
    if (!Array.isArray(fresh.clickerTiers) || fresh.clickerTiers.length === 0) {
      ...
      fresh.clickerTiers = Array.from({ length: 10 }, function (_, t) {
        return new Array(10).fill(t < legacyTiers ? 10 : 0);
      });
    }
    // Validate clickerTiers shape and clamp values
    if (!Array.isArray(fresh.clickerTiers)) {
      fresh.clickerTiers = Array.from({ length: 10 }, function () { return new Array(10).fill(0); });
    } else {
      for (let t = 0; t < 10; t++) {
